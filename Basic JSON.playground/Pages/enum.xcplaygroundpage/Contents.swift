...
Но если у вас 'enum' с ассоциативными значениями, как в случае с 'ContentKind', то вы должны взять на себя часть логики encode и decode.
 ...
 #### Во-вторых, вам необходимо сообщить Swift "ключи",  которые будут использоваться для encode и/или decode ваших данных. Для этого используется другое перечисление 'enum' 'CodingKeys', встроенное в протокол  'CodingKey'.
